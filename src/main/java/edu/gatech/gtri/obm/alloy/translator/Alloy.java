...
public class Alloy {
   ...
   * @param parameterFields - used to determine fields to be disj constraint (parameter files are
   *     not disj)
   ...
   * @param signatureBlockBySignature - Map (key = Signature name string, value = Signature and
   *     fields as written in the alloy file).
   * @return string of Signatures, Fields, and Facts grouped by Signature.
   */
  private static String format(
      String factListInString, Map<String, String> signatureBlockBySignature) {
   ...
   * String[] domainAndFacts for index = 1 or more of "fact {all x: MultipleControlFlow | no y:
   * Transfer | y in x.steps}" is ["fact {all x: MultipleControlFlow", "no y: Transfer", "y in
   * x.steps"], this methods return "no y: Transfer | y in x.steps".
   *
