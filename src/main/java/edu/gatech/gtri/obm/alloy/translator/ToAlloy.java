...
public class ToAlloy {
  ...
  protected PrimSig createSigOrReturnSig(String name, String parentName) {
    ...
    else parentSig = (PrimSig) sigByName.get(parentName);
  ...
  protected Sig.Field[] addDisjAlloyFields(
      List<String> fieldNamesListWithSameType, String fieldSigTypeName, PrimSig ownerSig) {
   ...
   * @param fieldOfsigFieldType - a field(property) of sigField type(class) (ie., i of p1's type
   *     signature(class)) = sourceOutputProperty or targetInputProperty
   * @param inputsOrOutputs - a function inputs or outputs
   */
  protected void createInField(
      Sig ownerSig, Sig.Field sigField, Sig.Field fieldOfsigFieldType, Func inputsOrOutputs) {
    alloy.addToFacts(
        AlloyExprFactory.exprs_inField(ownerSig, sigField, fieldOfsigFieldType, inputsOrOutputs));
  }

  /**
   * Create the bijection filtered happens before and add to the alloy's facts. The from and to
  ...
  protected void createBijectionFilteredHappensBefore(Sig ownerSig, Expr from, Expr to) {
    alloy.addToFacts(
        AlloyExprFactory.exprs_bijectionFilteredFactsForSig(
            ownerSig, from, to, Alloy.happensBefore));
  }

  /**
   * Create the bijection filtered happens during and add to the alloy's facts. The from and to
  ...
  protected void createBijectionFilteredHappensDuring(Sig ownerSig, Expr from, Expr to) {
    alloy.addToFacts(
        AlloyExprFactory.exprs_bijectionFilteredFactsForSig(
            ownerSig, from, to, Alloy.happensDuring));
  }

  /**
   * Create the bijection filtered inputs during and add the alloy's facts. The from and to
   * expression can be just a filed (i.e., p1, b.vin) or multiple fields connected by plus (p1 + p2
   * + p3).
   *
   * <pre>
  ...
  /**
   * Create the bijection filtered inputs during and add to the alloy's facts. The from and to
   * expression can be just a filed (i.e., p1, b.vin) or multiple fields connected by plus (p1 + p2
   * + p3).
   *
   * <pre>
   ...
   * @param to - the to expression
   */
  protected void createFunctionFilteredHappensBeforeAndAddToOverallFact(
      Sig ownerSig, Expr from, Expr to) {
    alloy.addToFacts(AlloyExprFactory.expr_functionFilteredHappensBeforeForSig(ownerSig, from, to));
  }
   ...
   * @param to - the to expression
   */
  protected void createInverseFunctionFilteredHappensBeforeAndAddToOverallFact(
      Sig ownerSig, Expr from, Expr to) {
    alloy.addToFacts(
   ...
   * if lower and upper are the same, add the equal fact constraint. if upper is -1, lower is
   ...
   * @return true if the fact added successfully, false if the fact can not add successfully because
   *     the field did not exist in the sig.
   */
  protected void addCardinalityFact(PrimSig sig, Field field, int lower, int upper) {
    if (lower == upper) alloy.addToFacts(AlloyExprFactory.expr_cardinalityEqual(sig, field, lower));
  ...
  protected boolean addCardinalityFact(PrimSig sig, String fieldName, int lower, int upper) {
    ...
    else addCardinalityFact(sig, field, lower, upper);
    return true;
  }
  ...
  protected Set<Expr> addTransferFacts(
    ...
    if (targetInputsSourceOutputsFields != null) {
      if (targetInputsSourceOutputsFields.get(0).size() > 0) {
        ...
        if (!toBeInherited) alloy.addToFacts(AlloyUtils.toSigAllFacts(ownerSig, factsWithoutSig));
      }
    ...
    alloy.addToFacts(
        AlloyExprFactory.exprs_bijectionFilteredFactsForSig(
            ownerSig, transfer, sourceTypeField, Alloy.sources));
    alloy.addToFacts(
        AlloyExprFactory.exprs_bijectionFilteredFactsForSig(
            ownerSig, transfer, targetTypeField, Alloy.targets));
   ...
   * no inputs.x}" based on the given addNoOutputsX and addEqual.
   *
   ...
   * @param addEquall - boolean if true, includes facts like "..in x.outputs" and "x.outputs in...",
   *     if false, not to includes
   */
  protected void addInInputsAndNoInputXFacts(
      PrimSig sig, Set<Field> fields, boolean addNoInputsX, boolean addEqual) {
   ...
   * @param addEqual - boolean if true, includes facts like "..in x.outputs" and "x.outputs in...",
   *     if false, not to includes
   */
  protected void addInOutputsAndNoOutputsXFacts(
      PrimSig sig, Set<Field> fields, boolean addNoOutputsX, boolean addEqual) {
   ...
   * x.prepare.preparedFoodItem + x.prepare.prepareDestination}"
   *
   * @param ownerSig - A signature of the fact
   * @param field - A field of the given owner signature used to create expression after "x." to
   ...
   * is not in noTransferInXStepsFactSigs, is leaf sig, and has own or inherited fields.
   *
  ...
  protected void addNoTransferInXStepsFact(
      Set<Sig> noTransferInXStepsFactSigs, Set<PrimSig> leafSigs) {
    Object[] sigs =
        sigByName.values().stream()
            .filter(sig -> !noTransferInXStepsFactSigs.contains(sig))
            .toArray();
    for (Object sig : sigs) {
   ...
   * transferingTypeSig (i.e., Integer) is a leaf Signature.
   *
   ...
   * </pre>
   *
   * @param stepPropertiesBySig - a map (key = signature name, value = a set of property names) of
   *     step properties by signature
   * @param leafSig - a set of signature that is leaf
   */
  protected Set<Sig> addStepsFacts(
      Map<String, Set<String>> stepPropertiesBySig, Set<PrimSig> leafSigs) {
    ...
    for (String sigName : stepPropertiesBySig.keySet()) {
      ...
      // if leaf Sig do
      if (leafSigs.contains(sig)) {
        if (stepPropertiesBySig.get(sigName).size()
            > 0) { // {x.steps in ....} and {... in x.steps} for leaf signature
          alloy.addToFacts(
              AlloyExprFactory.exprs_stepsFields(
                  sig, stepPropertiesBySig.get(sigName), true, true));
        } else {
          alloy.addToFacts(
      ...
      } else if (stepPropertiesBySig.get(sigName).size()
   ...
   * @param propertyNameAndType - A map where key is property name string and value is the property
   *     type (Class) name
   */
  protected void addRedefinedSubsettingAsFacts(
      PrimSig ownerSig, Map<String, String> propertyNameAndType) {
   ...
   *
   * @param sigNameWithTransferConnectorWithSameInputOutputFieldType {BehaviorParameterIn=[vin],
   *     BehaviorParameterInOut=[vin]}
  ...
  protected void handleNoInputsOutputs(
    ...
    Set<String> inputFlowFieldTypes =
        sigInputProperties.size() == 0
            ? null
            : getFlowTypeSig(sigInputProperties); // Sigs [Integer]
    // find what signatures are flowing out
    Set<String> outputFlowFieldTypes =
        sigOutputProperties.size() == 0
            ? null
            : getFlowTypeSig(sigOutputProperties); // Sigs [Integer]
    ...
    for (String sigName : sigNames) {
      ...
      if (sigInputProperties.keySet().contains(sigName)) {
        ...
        if (inputFlowFieldTypes != null
            && inputFlowFieldTypes.contains(sigName)) // Integer is flowing
   ...
   *
   * <p>For 4.1.4 Transfers and Parameters - ParameterBehavior.als, the inputs map is {B2=[vin],
   * B=[vin], C=[vin], B1= [vin] B2=[vout], A=[vout]} and this method returns [Real].
   *
   * <p>For 4.1.4 Transfers and Parameters - TransferProduct.als, the inputs map is {Customer =
   * [reveivedProduct]} and this method returns [Product]. Its output maps is {Supplier
   * =[suppliedProduct]} and this method returns [Product]
   *
   * <p>For 4.2.2 Food Service Object Flow - OFParallelFoodService.als, the inputs map is
   * {OFServe=[servedFoodItem], OFCustomPrepare=[preparedFoodItem, prepareDestination],
   * OFEat=[eatenItem], OFPay=[paidFoodItem, paidAmount], OFCustomServe=[servedFoodItem,
   * serviceDestination]} and this method returns [FoodItem, Real, Location]. Its outputs map is
   * {OFCustomOrder=[orderedFoodItem, orderAmount, orderDestination], OFOrder=[orderedFoodItem],
   * OFCustomPrepare=[preparedFoodItem, prepareDestination], OFPay=[paidFoodItem],
   * OFCustomServe=[servedFoodItem]} and this method returns [FoodItem, Real, Location]
   *
   * @param inputsOrOutputs - a map where key is a signature name string and value is a set of its
   *     input or output field name strings.
