...
public class ConnectorsHandler_OneOf {
  ...
  protected Set<Connector> handleOneOfConnectors(
      PrimSig _sigOfClass, Class _classOfSig, Set<org.eclipse.uml2.uml.Connector> connectors) {
    ...
    Set<EList<Element>> oneOfSets =
        AlloyUtils.getOneOfRules(
            sysmladapter, constraints); // EList<ConnectorEnd> [ [start, eat] or [order, end]]
   ...
   *
   * @param oneOfSet List of ConnectableElement both having oneOf constraint. Both should be
   ...
   * @param oneSourceProperties list of connectableElements of the sig's one of connectors which has
   *     only one outgoing.
   * @param oneTargetProperties list of connectableElements of the sig's one of connectors which has
   *     only one incoming.
   */
  ...
  private void handleSourceSideOneOf(
      List<ConnectableElement> sources,
      String targetName,
      PrimSig _sigOfClass,
      Set<ConnectableElement> _oneSourceProperties) {
    ...
    afterExpr = AlloyUtils.getFieldFromSigOrItsParents(targetName, _sigOfClass);
    ...
    if (allSourceOneOf) { // merge a + b -> c
      this.toAlloy.createBijectionFilteredHappensBefore(_sigOfClass, beforeExpr, afterExpr);
    } else {
      for (ConnectableElement ce : _oneSourceProperties) {
        // need fn a -> b or a ->c
  ...
  private void handleTargetSideOneOf(
      List<ConnectableElement> targets,
      String sourceName,
      PrimSig _sigOfClass,
    ...
    beforeExpr = AlloyUtils.getFieldFromSigOrItsParents(sourceName, _sigOfClass);
    ...
    if (allTargetOneOf) { // decision a -> b + c
      ...
      this.toAlloy.createFunctionFilteredHappensBeforeAndAddToOverallFact(
          _sigOfClass, beforeExpr, afterExpr);
